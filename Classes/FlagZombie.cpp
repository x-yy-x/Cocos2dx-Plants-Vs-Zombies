...
// ----------------------------------------------------
const std::string FlagZombie::IMAGE_FILENAME = "flag_zombie_walk_spritesheet.png";
const cocos2d::Rect FlagZombie::INITIAL_PIC_RECT = Rect::ZERO;
const cocos2d::Size FlagZombie::OBJECT_SIZE = Size(125.0f, 173.8f);
const float FlagZombie::MOVE_SPEED = 20.0f;       // Move 20 pixels per second
const float FlagZombie::ATTACK_DAMAGE = 10.0f;    // Damage per attack
const float FlagZombie::ATTACK_RANGE = 50.0f;     // Attack range
...
void FlagZombie::initWalkAnimation()
{
    ...
    for (int row = 0; row < 3; row++)
    {
        for (int col = 0; col < 5; col++)
        {
            ...
            auto frame = SpriteFrame::create(
                "flag_zombie_walk_spritesheet.png",
    ...
    auto animation = Animation::createWithSpriteFrames(frames, 0.18f);
...
void FlagZombie::initEatAnimation()
{
    ...
    for (int row = 0; row < 3; row++)
    {
        for (int col = 0; col < 5; col++)
        {
            ...
            auto frame = SpriteFrame::create(
                "flag_zombie_eat_spritesheet.png",
                Rect(x, y, frameWidth, frameHeight)
            );
...
// Update every frame
void FlagZombie::update(float delta)
{
    if (_isDead || _isDying)
    {
        return;
...
// Get zombie state
FlagZombie::ZombieState FlagZombie::getState() const
{
    CCLOG("getstate����");
...
void FlagZombie::encounterPlant(const std::vector<Plant*>& plants)
{
    checkCollision(plants);
}

// Check collision with plants
void FlagZombie::checkCollision(const std::vector<Plant*>& plants)
{
    if (_isEating) return;
...
void FlagZombie::startEating(Plant* plant)
{
    _isEating = true;
    _targetPlant = plant;
